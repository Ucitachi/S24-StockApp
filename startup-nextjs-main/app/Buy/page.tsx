...
function Buy() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {   
                setPopupMessage('Stock Bought successfully!'); 
                setShowSuccessPopup(true);
                setTimeout(() => {
                  router.push('/dashboard');
                }, 1500);
              } else if (response.status === 206) { // Check for response.data.error
              ...
              setPopupMessage('An error occurred. Please try again.'); // Generic message
              setShowSuccessPopup(true); 
            }
          };
    ...
    return (
        <div className="w-full px-4 container mx-auto mt-20">
    <div
    ...
    {showSuccessPopup && ( 
  <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black p-5 rounded-md shadow-lg z-50"> 
    {popupMessage} 
  </div> 
)}
        </div>
